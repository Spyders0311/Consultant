...
export function createEngineCalculateHandler(enginePath) {
  return async function POST(request) {
    ...
    if (isProduction && (isMissing || isLocalOrDefault)) {
      return Response.json({ ok: false, error: ENGINE_CONFIG_ERROR }, { status: 500 });
    }

    try {
      const body = await request.json();
      const upstream = await fetch(`${PYTHON_ENGINE_URL}${enginePath}`, {
        method: 'POST',
      ...
      return Response.json({ ok: false, error: 'Unable to reach calculation engine.' }, { status: 502 });
    }
  };
}

/**
 * Server-side call to the Python calculation engine.
 *
 * @param {string} enginePath
 * @param {Record<string, unknown>} body
 */
export async function callEngineCalculate(enginePath, body) {
  const upstream = await fetch(`${PYTHON_ENGINE_URL}${enginePath}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body || {}),
  ...
  const data = await upstream.json();
  if (!upstream.ok || !data.ok) {
    throw new Error(data.error || 'Python engine rejected calculation.');
  }
  return data.result;
}
