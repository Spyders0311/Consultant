...
export async function buildResultsPdf({ model, result, consultant }) {
  ...
  } else if (model === 'breakeven') {
    addSummaryRow(doc, 'Gross Margin Amount', currency(result.grossMarginAmount), summaryY + 30);
    addSummaryRow(doc, 'Gross Margin Percent', percentValue(result.grossMarginPercent), summaryY + 52);
    addSummaryRow(doc, 'Breakeven Revenue', currency(result.breakevenRevenue), summaryY + 74);
    addSummaryRow(doc, 'Breakeven % of Current Revenue', percentValue(result.breakevenPercent), summaryY + 96);

    const startY = summaryY + 150;
    doc.fillColor('#334155').font('Helvetica-Bold').fontSize(13).text('Breakeven Run-Rate Targets', 48, startY);
    addSummaryRow(doc, 'Monthly', currency(result.breakevenMonthly), startY + 26);
    addSummaryRow(doc, 'Weekly', currency(result.breakevenWeekly), startY + 48);
    addSummaryRow(doc, 'Daily', currency(result.breakevenDaily), startY + 70);
    addSummaryRow(doc, 'Hourly', currency(result.breakevenHourly), startY + 92);

    const notes = result.notes || [];
    if (notes.length) {
      let y = startY + 140;
      doc.font('Helvetica-Bold').fillColor('#991b1b').fontSize(12).text('Notes & Warnings', 48, y);
      y += 16;
      doc.font('Helvetica').fillColor('#7f1d1d').fontSize(10);
      for (const note of notes) {
        doc.text(`- ${note}`, 56, y, { width: 470 });
        y += 14;
        if (y > 760) break;
      }
    }
