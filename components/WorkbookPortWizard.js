...
function makeDefaultForm(config) {
  ...
  for (const collection of config.collections || []) {
    form[collection.key] = (collection.defaultRows || []).map((row) => ({ ...row }));
  }
  return form;
}
...
function normalizePayload(config, form) {
  ...
  for (const collection of config.collections || []) {
    payload[collection.key] = (form[collection.key] || []).map((row, index) => {
      const normalized = {};
      for (const field of collection.fields) {
        const fallback = field.name === 'period' || field.name === 'weekLabel' ? `${collection.rowLabel} ${index + 1}` : '';
        normalized[field.name] = normalizeValue(field, row[field.name] ?? fallback);
      }
      return normalized;
    });
  }
...
function mergeRunForm(config, inputs) {
  ...
  for (const collection of config.collections || []) {
    merged[collection.key] = Array.isArray(inputs?.[collection.key]) ? inputs[collection.key] : base[collection.key];
  }
  return merged;
}
...
function InputControl({ field, value, onChange, disabled }) {
...
function historyMetric(config, outputs) {
  const metric = config.historyMetric;
  if (!metric) return 'saved output';
...
export default function WorkbookPortWizard({ clientId, workbookKey }) {
  ...
  const canRun = useMemo(() => hasMeaningfulInput(form), [form]);
  ...
  function addCollectionRow(collection) {
    setForm((prev) => {
      ...
      for (const field of collection.fields) {
        nextRow[field.name] =
          field.type === 'select' ? field.options?.[0]?.value || '' : field.name === 'weekLabel' || field.name === 'period' ? `${collection.rowLabel} ${rows.length + 1}` : '';
      }
  ...
  function loadRun(run) {
    if (!run) return;
    setForm(mergeRunForm(config, run.inputs || {}));
  ...
  useEffect(() => {
    ...
    async function fetchRuns() {
      ...
      setRunsLoading(true);
      try {
        const params = new URLSearchParams({ client_id: clientId, workbook_key: workbookKey });
        const response = await fetch(`/api/worksheets/workbook-ports/runs?${params.toString()}`, { cache: 'no-store' });
        const data = await response.json();
        if (!response.ok || !data.ok) {
          throw new Error(data.error || 'Unable to load workbook history.');
        }
        if (!cancelled) setRuns(data.runs || []);
      } catch (err) {
        if (!cancelled) setError(err.message || 'Unable to load workbook history.');
      } finally {
  ...
  async function calculateAndSave(event) {
    ...
    try {
      ...
      const calculationResponse = await fetch('/api/worksheets/workbook-ports/calculate', {
        ...
        body: JSON.stringify({ workbookKey, inputs: payload }),
      });
  ...
  if (!config) {
    return (
      <section className="panel">
        <h2>Workbook unavailable</h2>
        <p>This workbook port is not configured.</p>
      </section>
    );
  }
  ...
  return (
    <section className="wizard-shell">
      <header className="wizard-header">
        ...
        <p className="wizard-meta">Source: {config.source}</p>
      </header>
        ...
        <div className="wizard-fields">
          {(config.scalarFields || []).map((field) => (
            <label key={field.name}>
              {field.label}
              <InputControl field={field} value={form[field.name]} onChange={(value) => updateField(field.name, value)} disabled={loading} />
            </label>
          ))}
        </div>

        {(config.collections || []).map((collection) => (
          <section className="workbook-port-collection" key={collection.key}>
            <div className="workbook-port-collection-header">
              <h2>{collection.label}</h2>
              <button type="button" className="ghost" onClick={() => addCollectionRow(collection)} disabled={loading || (form[collection.key] || []).length >= collection.maxRows}>
            ...
            <div className="table-wrap worksheet-grid-table">
              <table>
                ...
                <tbody>
                  {(form[collection.key] || []).map((row, rowIndex) => (
                      ...
                      {collection.fields.map((field) => (
                        <td key={field.name}>
                          <InputControl
                            ...
                            onChange={(value) => updateCollectionRow(collection.key, rowIndex, field.name, value)}
                            disabled={loading}
                          />
                        </td>
                      ))}
                      <td>
                        <button type="button" className="ghost" onClick={() => removeCollectionRow(collection.key, rowIndex)} disabled={loading || (form[collection.key] || []).length <= 1}>
                          Remove
                        </button>
                      </td>
                    </tr>
          ...
          <div className="wizard-kpis">
            {(config.summaryFields || []).map((field) => (
              <article key={field.key}>
                ...
                <strong>{formatValue(result.summary?.[field.key], field.type)}</strong>
              </article>
            ))}
          </div>
          ...
          {rows.length ? (
            <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {rows.map((row, index) => (
                    <tr key={`${workbookKey}-result-${index + 1}`}>
                      {(config.tableFields || []).map((field) => (
                        <td key={field.key}>{formatValue(row[field.key], field.type)}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
      ...
      <aside className="run-history">
        ...
        {!runsLoading && runs.length === 0 ? <p>No saved runs yet.</p> : null}
        <div className="run-list">
          {runs.map((run) => (
            <button type="button" className="run-list-item" key={run.id} onClick={() => loadRun(run)}>
              ...
              <span>{historyMetric(config, run.outputs)}</span>
            </button>
          ))}
        </div>
      </aside>
