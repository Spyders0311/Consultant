...
export async function GET(request) {
  ...
  const { data, error } = await supabase
    .from('client_flexible_budget_variance_runs')
  ...
  if (error) {
    return NextResponse.json(
      { ok: false, error: error.message || 'Failed to load flexible budget variance runs.' },
      { status: 500 },
    );
  }
...
export async function POST(request) {
  ...
  if (error) {
    return NextResponse.json(
      { ok: false, error: error.message || 'Failed to save flexible budget variance run.' },
      { status: 500 },
    );
  }
