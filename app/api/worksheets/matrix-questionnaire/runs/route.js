...
export async function GET(request) {
  ...
  const matrixKey = searchParams.get('matrix_key') || searchParams.get('matrixKey');
  if (!clientId) return Response.json({ ok: false, error: 'Missing client_id.' }, { status: 400 });

  let query = supabase
    .from('client_matrix_questionnaire_runs')
    .select('id, created_at, matrix_key, inputs, outputs')
    .eq('client_id', clientId)
    .order('created_at', { ascending: false })
    .limit(parseLimit(searchParams.get('limit')));
  if (matrixKey) query = query.eq('matrix_key', matrixKey);
...
export async function POST(request) {
  ...
  const matrixKey = payload?.matrix_key || payload?.matrixKey;
  if (!clientId || !matrixKey) {
    return Response.json({ ok: false, error: 'Missing client_id or matrix_key.' }, { status: 400 });
  }
