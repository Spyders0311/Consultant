...
export async function GET(request) {
  ...
  const { data, error } = await supabase
    .from('client_five_year_projections_runs')
  ...
  if (error) {
    return NextResponse.json(
      { ok: false, error: error.message || 'Failed to load five year projection runs.' },
      { status: 500 },
    );
  }
...
export async function POST(request) {
  ...
  if (error) {
    return NextResponse.json(
      { ok: false, error: error.message || 'Failed to save five year projection run.' },
      { status: 500 },
    );
  }
